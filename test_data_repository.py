#!/usr/bin/env python3
"""
Test script for the data repository system
"""
...
def test_data_exporters(repo):
    ...
    for file_path in csv_files:
        assert os.path.exists(file_path), f"CSV file {file_path} should exist"
...
def test_simulation_data_analysis(repo):
    """Test data analysis functionality"""
    print("\nTesting Data Analysis...")

    from av_simulation.data.repository import SimulationData

    data_analyzer = SimulationData(repo)
    ...
    if runs:
        ...
        ego_trajectory = data_analyzer.get_ego_vehicle_data(run_id)
        assert len(ego_trajectory) > 0, "Ego vehicle trajectory should not be empty"
        print(f"✓ Found {len(ego_trajectory)} ego vehicle data points")

        # Test speed profile
        speed_profile = data_analyzer.get_speed_profile(run_id, "ego_0")
        assert len(speed_profile) > 0, "Speed profile should not be empty"
        print(f"✓ Speed profile has {len(speed_profile)} data points")

        # Test collision analysis
        collision_analysis = data_analyzer.get_collision_analysis(run_id)
        assert collision_analysis["total_collisions"] == 1, "Should have 1 collision"
        print(f"✓ Collision analysis: {collision_analysis['total_collisions']} collisions detected")
...
def main():
    ...
    try:
        ...
        # Test data analysis
        test_simulation_data_analysis(repo)
